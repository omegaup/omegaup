<?php
	...
	if ($_POST) {
		if (!in_array($_POST['language'], array('c', 'cpp', 'java'))) {
		...
		} else if (!in_array($_POST['os'], array('unix', 'windows'))) {
			$smarty->assign('error', $smarty->getConfigVars('parameterInvalid'));
			$smarty->assign('error_field', 'os');
		} else if (!preg_match('/^[a-z_][a-z0-9_]{0,31}$/i', $_POST['name'])) {
			$smarty->assign('error', $smarty->getConfigVars('parameterInvalid'));
			$smarty->assign('error_field', 'name');
		} else if (empty($_POST['idl'])) {
			$smarty->assign('error', $smarty->getConfigVars('parameterInvalid'));
			...
			try {
				...
				$proc = proc_open($cmd, $descriptorspec, $pipes, $dirname,
				...
				if (!is_resource($proc)) {
					...
					if ($retval != 0) {
						...
						$zip = new ZipArchive();
						$zip->open("{$dirname}/interactive.zip",
						...
						foreach ($files as $name => $file)
						{
							...
							if ($file->getFilename() == 'interactive.zip') continue;
