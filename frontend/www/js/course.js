omegaup.OmegaUp.on('ready', function() {
  ...
  omegaup.API.getCourseDetails({ alias: courseAlias }).then(function(course) {
    ...
    for (var i = 0; i < course.assignments.length; ++i) {
      ...
      course.assignments[i].assignmentUrl = '/course/' +
        courseAlias +
        '/assignment/' +
        course.assignments[i].alias;
      course.assignments[i].startTime = omegaup.UI.formatDateTime(new Date(
        1000 * course.assignments[i].start_time
      ));
      course.assignments[i].finishTime = omegaup.UI.formatDateTime(new Date(
        1000 * course.assignments[i].finish_time
      ));
    }
