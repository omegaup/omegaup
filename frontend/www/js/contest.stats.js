$('document').ready(function() {
	var contestAlias = /\/contest\/([^\/]+)\/stats\/?.*/.exec(window.location.pathname)[1];
	...
	function drawCharts() {
		...
		window.run_counts_chart = oGraph.verdictCounts('verdict-chart', contestAlias, stats);

		// Draw distribution of scores chart
		window.distribution_chart = oGraph.distributionChart('distribution-chart', contestAlias, stats);
	}
