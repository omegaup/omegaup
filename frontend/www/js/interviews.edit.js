omegaup.OmegaUp.on('ready', function() {
  ...
  omegaup.API
    .getContestAdminDetails({ contest_alias: contestAlias })
    .then(function(contest) {
      $('.page-header h1 span').html(
        omegaup.T['interviewEdit'] + ' ' + contest.title
      );
  ...
  function refreshContestProblems() {
    omegaup.API
      .contestProblems({ contest_alias: interviewAlias })
      .then(function(response) {
  ...
  omegaup.API.getProblems().then(function(problems) {
    ...
    for (var i = 0; i < problems.results.length; i++) {
      ...
      $('select#problems').append(
        $('<option></option>').attr('value', problem.alias).text(problem.title)
      );
    }
  });
  ...
  function refreshContestAdmins() {
    omegaup.API
      .getContestAdmins({ contest_alias: interviewAlias })
