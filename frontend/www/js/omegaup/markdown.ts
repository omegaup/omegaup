...
export class Converter {
  ...
  constructor(options: ConverterOptions = {}) {
    ...
    const whitelist = /^<\/?(a(?: (target|class|href)="[a-z/_-]+")*|figure|figcaption|code|i|table|tbody|thead|tr|th(?: align="\w+")?|td(?: align="\w+")?|iframe(?: (width|height)="\d+")*(?: src="https:\/\/www.youtube.com\/embed\/[\w\-]+")?(?: frameborder="[01]+")?(?: allow="[a-zA-Z-_;[:space:]]+")*(?: allowfullscreen(="(true|false)+")*)*|div|h3|span|form(?: role="\w+")*|label|select|option(?: (value|selected)="\w+")*|strong|span|button(?: type="\w+")?)( class="[a-zA-Z0-9 _-]+")?>$/i;
