<template>
  <div class="user-heatmap-container">
    <div class="heatmap-title">
      <template v-if="T && T.activityHeatmapFor">
        {{ ui.formatString(T.activityHeatmapFor, { alias: username }) }}
      </template>
      <template v-else> Activity heatmap for {{ username }} </template>
    </div>
...
export default class UserHeatmap extends Vue {
  ...
  renderHeatmap() {
    ...
    this.chart = Highcharts.chart(this.$refs.heatmapContainer, {
      chart: {
        type: 'heatmap',
        height: chartHeight,
        width: chartWidth,
      ...
      tooltip: {
        formatter: function (this: Highcharts.TooltipFormatterContextObject) {
          ...
          if (value > 0) {
            if (T && T.totalSubmissions) {
              return `<b>${formattedDate}</b><br>${ui.formatString(
                T.totalSubmissions,
                {},
              )}: <b>${value}</b>`;
            } else {
