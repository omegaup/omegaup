<template>
	<div>
		<div v-for="contestantData in contestReport">
			...
			<h1>Total:<span v-if="contestantData.hasOwnProperty('total') && contestantData.total.hasOwnProperty('points')">{{ contestantData.total.points }}</span><span v-else="">0</span></h1>
			<div v-for="(item,key) in contestantData.problems">
				<h4>Problem: {{ key }}</h4>
				<h4>Points: {{ item.points }}</h4>
				<table v-if="item.run_details && (((item || {}).run_details || {} ).details || {} ).groups">
					<tr>
						<th>Case</th>
						<th>Time (Sec)</th>
						<th>Time-wall (Sec)</th>
					...
					<template v-for="group in item.run_details.details.groups">
						<tr v-for="temp in group.cases">
							...
							<td>{{ temp.verdict }}</td>
							<td>{{ temp.score }}</td>
							<td><template v-if="temp.out_diff">{{ temp.out_diff }}</template></td>
						</tr>
					</template>
				</table>
				<table v-if="item.run_details && (((item || {}).run_details || {} ).details || {} ).groups">
...
export default {
	...
	mounted: function(){
		console.log(this.contestReport)
	}
};
