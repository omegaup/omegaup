...
describe('List.vue', () => {
  ...
  describe('Sort controls', () => {
    it('Should sort by problem title by default', async () => {
      let nominations : types.NominationListItem[] = [];
    ...
    it('Should sort by problem title in descending order', async () => {
      ...
      const sortControlByTime = wrapper.findComponent({ ref: 'sortControlByTitle' });
    ...
    expect(wrapper.vm.orderedNominations[0].problem.title).toBe('Problem 1');
    expect(wrapper.vm.orderedNominations[0].time.getTime()).toBe(new Date('2021-02-06 00:00:00').getTime());

    const sortControlByTime = wrapper.findComponent({ ref: 'sortControlByTime' });
    expect(sortControlByTime.exists()).toBe(true);

    sortControlByTime.vm.$emit('apply-filter', 'time', 'asc');
    expect(wrapper.vm.orderedNominations[0].problem.title).toBe('Problem 6');
    expect(wrapper.vm.orderedNominations[0].time.getTime()).toBe(new Date('2021-02-01 00:00:00').getTime());
  });
  });
});
