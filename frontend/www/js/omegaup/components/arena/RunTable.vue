    ...
    <table class="runs">
        <caption>
            ...
            <div class="runspager" v-if="showPager">
                ...
                <label v-if="showProblem">{{T.wordsProblem + ':'}}
                    <omegaup-autocomplete v-bind:init="el => isContest ? UI.problemContestTypeahead(el) : UI.problemTypeahead(el)" v-model="filterProblem"></omegaup-autocomplete>
                </label>
                <button type="button" class="close runsproblem-clear" style="float: none;" v-on:click="filterProblem = ''" v-if="showProblem">&times;</button>

                <label v-if="showUser">{{T.wordsUser + ':'}}
                    <omegaup-autocomplete v-bind:init="el => UI.userTypeahead(el)" v-model="filterUsername"></omegaup-autocomplete>
                </label>
                <button type="button" class="close runsusername-clear" style="float: none;" v-if="showUser">&times;</button>
            </div>
        </caption>
        <thead>
            <tr>
                ...
                <th>{{T.wordsStatus}}</th>
                <th class="numeric" v-if="showPoints">{{T.wordsPoints}}</th>
                <th class="numeric" v-if="showPoints">{{T.wordsPenalty}}</th>
                <th class="numeric" v-else>{{T.wordsPercentage}}</th>
        ...
        <tbody>
            <tr v-for="run in runs">
                ...
                <td><acronym :title="run.guid">{{shortGUID(run.guid)}}</acronym></td>
                <td v-if="showUser">{{run.username}}</td>
                <td v-if="showContest"><a :href="UI.getContestURL(run.contest_alias)">{{run.contest_alias}}</a></td>
                <td v-if="showProblem"><a :href="UI.getProblemURL(run.alias)">{{run.alias}}</a></td>
                <td :style="{ backgroundColor: statusColor(run) }">
                ...
                <td>{{runtime(run)}}</td>
                <td v-if="showRejudge">
                    <button type="button" class="glyphicon glyphicon-repeat" title="rejudge" v-on:click="rejudge"></button>
                    <button type="button" class="glyphicon glyphicon-flag" title="debug" v-on:click="debug_rejudge"></button>
                </td>
                <td v-if="showDisqualify">
                    <button type="button" class="glyphicon glyphicon-ban-circle" title="disqualify" v-on:click="disqualify"></button>
                </td>
                <td v-if="showDetails">
                    <button type="button" class="details glyphicon glyphicon-zoom-in"></button>
                </td>
            </tr>
        </tbody>
        <tfoot v-if="showSubmit">
            <tr>
                <td colspan="9"><a href="#problems/new-run">{{T.wordsNewSubmissions}}</a></td>
                <td colspan="9" style="display:none"><a>{{T.arenaContestEndedUsePractice}}</a></td>
            </tr>
        </tfoot>
    </table>
</template>
    ...
    methods: {
        ...
        memory: function(run) {
            if (run.status == 'ready' && run.verdict != 'JE' && run.verdict != 'CE') {
                ...
                return prefix + (parseFloat(run.memory) / (1024 * 1024)).toFixed(2) + 'Mb';
            } else {
                return '—';
            }
        },
        runtime: function(run) {
            if (run.status == 'ready' && run.verdict != 'JE' && run.verdict != 'CE') {
                ...
                return prefix + (parseFloat(run.runtime || '0') / 1000).toFixed(2) + 's';
            } else {
                return '—';
            }
        }
