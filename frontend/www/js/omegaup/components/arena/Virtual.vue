...
export default {
    ...
    data: function() {
        return {
            ...
            detail: {},
            start_time: new Date(),
            contest_alias:  /\/arena\/([^\/]+)\/virtual/.exec(window.location.pathname)[1]
        }
    },
    created: function() {
        var self = this;

        API.Contest.details({contest_alias: this.contest_alias}).then(function(response){
            var detail = response
    ...
    methods: {
        onSubmit: function() {
            API.Contest.createVirtual({alias: this.contest_alias, start_time: this.start_time.getTime() / 1000}).then(function(response) {
