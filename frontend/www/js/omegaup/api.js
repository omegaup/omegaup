  ...
  getCourseList: function(callback) {
    ...
    function(result) {
      for (var i = 0; i < result.admin.length; ++i) {
        result.admin[i].finish_time = omegaup.OmegaUp.time(result.admin[i].finish_time * 1000);
      }
      for (var i = 0; i < result.student.length; ++i) {
        result.student[i].finish_time = omegaup.OmegaUp.time(result.student[i].finish_time * 1000);
      }
      return result;
    });
  },
