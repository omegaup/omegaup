...
OmegaUp.on('ready', () => {
  ...
  function updateProblemScore(
    ...
    const scoreboardRanking = scoreboard.ranking.map((rank) => {
      const ranking = rank;
      if (ranking.username === commonPayload.currentUsername) {
  ...
  function onRankingChanged(
    ...
    for (const [i, rank] of ranking.entries()) {
      ...
      for (const alias of Object.keys(order)) {
        ...
        if (
          problems[alias] &&
          rank.username === commonPayload.currentUsername &&
          ...
          updateProblemScore(alias, currentPoints, problem.points, scoreboard);
        }
      }
