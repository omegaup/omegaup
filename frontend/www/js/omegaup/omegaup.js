...
export class Experiments {
  ...
  static loadGlobal() {
    var experimentsNode = document.getElementById(
...
export let OmegaUp = {
      ...
      function() {
        ...
        reportAnIssue.addEventListener('click', function(event) {
          ...
          let issueBody = T.reportAnIssueTemplate
            ...
            .replace('%(serializedErrors)', JSON.stringify(OmegaUp._errors))
            .replace(/\\n/g, '\n');
          reportAnIssue.href =
  ...
  remoteTime: function(timestamp, options) {
    options = options || {};
    options.server_sync =
  ...
  convertTimes: function(item) {
    ...
    if (item.hasOwnProperty('submission_deadline')) {
      item.submission_deadline = OmegaUp.remoteTime(
