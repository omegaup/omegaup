omegaup.OmegaUp.on('ready', function() {
  ...
  function refreshProblemAdmins() {
    omegaup.API
      .getProblemAdmins({ problem_alias: problemAlias })
  ...
  function refreshProblemTags() {
    omegaup.API
      .getProblemTags({ problem_alias: problemAlias })
  ...
  md_editor = new Markdown.Editor(md_converter, '-statement');
  // Global.
  md_editor.run();
  ...
  function problemCallback(problem) {
    $('.page-header h1 span').html(
      omegaup.T.problemEditEditProblem + ' ' + problem.title
    );
    $('.page-header h1 small').html(
      '&ndash; <a href="/arena/problem/' +
        problemAlias +
        '/">' +
        omegaup.T.problemEditGoToProblem +
        '</a>'
    );
