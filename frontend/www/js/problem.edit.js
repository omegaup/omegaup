$(document)
    .ready(function() {
      ...
      function problemCallback(problem) {
        ...
        $('input[name=public][value=' + problem.public + ']')
            .attr('checked', 1);
      ...
      $('#statement-preview-link')
          .on('show.bs.tab', function(e) {
            MathJax.Hub.Queue(
                ['Typeset', MathJax.Hub, $('#wmd-preview').get(0)]);
          });

      $('#statement-language')
          .on('change', function(e) {
            chosenLanguage = $('#statement-language').val();
            omegaup.API.getProblem(null, problemAlias, problemCallback,
                                   'markdown', false, chosenLanguage);
          });
    });
