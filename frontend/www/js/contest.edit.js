omegaup.OmegaUp.on('ready', function() {
  ...
  var contestAlias = /\/contest\/([^\/]+)\/edit\/?.*/.exec(
  ...
  omegaup.API
    ...
    .then(function(contest) {
      $('.page-header h1 span').html(
        omegaup.T['contestEdit'] + ' ' + contest.title
      );
      $('.page-header h1 small').html(
        '&ndash; <a href="/arena/' +
          contestAlias +
          '/">' +
          omegaup.T['contestDetailsGoToContest'] +
          '</a>'
      );
      ...
      $('.new_contest_form #start-time').val(
        omegaup.UI.formatDateTime(contest.start_time)
      );
      $('.new_contest_form #finish-time').val(
        omegaup.UI.formatDateTime(contest.finish_time)
      );
      ...
      if (
        contest.contestant_must_register == null ||
  ...
  // Update contest
  function updateContest(isPublic) {
        ...
        if (data.status == 'ok') {
          omegaup.UI.success(
            'Tu concurso ha sido editado! <a href="/arena/' +
              ...
              '">' +
              omegaup.T['contestEditGoToContest'] +
              '</a>'
          );
  ...
  function refreshContestProblems() {
    omegaup.API
      .contestProblems({ contest_alias: contestAlias })
        ...
        },
        {
          ...
          formatter: function(a, b, c) {
            return '<button type="button" ' +
              'class="close request-deny" value="' +
              b.username +
              '" style="color:red">&times;</button>' +
              '<button type="button" ' +
              'class="close request-accept" value="' +
              b.username +
  ...
  function refreshContestContestants() {
    omegaup.API
      .getContestUsers({ contest_alias: contestAlias })
      .then(function(users) {
        $('#contest-users').empty();
              ...
              .append(
                $('<td></td>').append(
                  $('<a></a>')
                    ...
                    .text(user.username)
                    .append(omegaup.UI.getFlag(user['country_id']))
                )
              )
  ...
  function refreshContestAdmins() {
    omegaup.API
      .getContestAdmins({ contest_alias: contestAlias })
