...
$(function() {
  if (formPage === 'list') {
      ...
      omegaup.API
        ...
        .then(function(data) {
          window.location.replace(
            '/group/' + $('.new_group_form #alias').val() + '/edit/#members'
          );
        })
        .fail(omegaup.UI.apiError);
    ...
    $('#add-member-form').submit(function() {
      ...
      omegaup.API
        .addUserToGroup({ group_alias: groupAlias, usernameOrEmail: username })
    ...
    function refreshGroupMembers() {
      omegaup.API
        .getGroupMembers({ group_alias: groupAlias })
        .then(function(group) {
          $('#group-members').empty();
    ...
    function refreshGroupScoreboards() {
      omegaup.API
        .getGroup({ group_alias: groupAlias })
        .then(function(group) {
