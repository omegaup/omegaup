    ...
    if (!result.error) {
      ...
      for (var i = 0; i < result.feed.entries.length; i++) {
        ...
        publishedDate.appendChild(
            document.createTextNode('Por: ' + entry.author));
        publishedDate.appendChild(document.createTextNode(
            ' - ' + Date.create(entry.publishedDate).short('es')));
        div.appendChild(publishedDate);

        var body = document.createElement('p');
        body.className = 'body';
        body.innerHTML =
            '<br>' + entry.content.slice(0, entry.content.indexOf('</p>') - 1) +
            "<b><a href='" + entry.link + "'>... Ver más > </a></b>";
