(function() {
    ...
    deferred.then(function(result) {
      ...
      for (var i = 0; i < result.contests.length; i++) {
        ...
        html += '<tr>' +
          ...
          '</a></b></td>' +
          '<td><a href="' +
          makeWorldClockLink(startDate) +
          '">' +
          startDate.format('long', 'es') +
          '</a></td>' +
          '<td><a href="' +
          makeWorldClockLink(endDate) +
          '">' +
          endDate.format('long', 'es') +
          '</a></td>' +
          '<td>' +
          (contest.public == '1'
            ? omegaup.T['wordsYes']
  ...
  $('#bulk-make-public').click(function() {
    omegaup.UI.bulkOperation(
      function(alias, handleResponseCallback) {
        omegaup.API
          .updateContest({ contest_alias: alias, public: 1 })
  ...
  $('#bulk-make-private').click(function() {
    omegaup.UI.bulkOperation(
      function(alias, handleResponseCallback) {
        omegaup.API
          .updateContest({ contest_alias: alias, public: 0 })
          .then(handleResponseCallback);
      },
      function() {
        fillContestsTable();
