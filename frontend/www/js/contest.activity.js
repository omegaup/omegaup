omegaup.OmegaUp.on('ready', function() {
  ...
  var ActivityReport = function(report) {
    ...
    for (var idx in self.events) {
      ...
      self.events[idx].time = Highcharts.dateFormat(
        '%Y-%m-%d %H:%M:%S',
        self.events[idx].time
      );
      if (self.events[idx].event.problem) {
        self.events[idx].event.problem_url = '/arena/problem/' +
          self.events[idx].event.problem +
          '/';
      }
    }
  ...
  omegaup.API
    .getContestActivityReport({ contest_alias: contestAlias })
