omegaup.OmegaUp.on('ready', function() {
  ...
  if ($('.requests-user-information').length) {
    var markdownConverter = omegaup.Markdown.markdownConverter();
    var payload = JSON.parse(document.getElementById('payload').innerText);
    document.getElementsByClassName(
      'requests-user-information',
    )[0].innerHTML = markdownConverter.makeHtml(payload['markdown']);
  }
  ...
  function enableStartContestButton() {
    ...
    if (
      $formElement.hasClass('requests-user-information-required') &&
  ...
  function readyToStart(contest) {
    ...
    if (now > contest.finish_time.getTime()) {
      // Ended
      $('#click-to-proceed').removeClass('hidden');
    } else if (now > contest.start_time.getTime()) {
      // Started
      $('#click-to-proceed').removeClass('hidden');
    } else {
      // Not started
  ...
  omegaup.API.Contest.publicDetails({ contest_alias: contestAlias })
    .then(function(contest) {
      ...
      if (contest.show_scoreboard_after) {
        $('.contest #show-scoreboard-after').text(
          omegaup.T.contestNewFormScoreboardAtContestEnd,
        );
      } else {
        $('.contest #show-scoreboard-after').hide();
      }
      if (contest.window_length != null) {
        $('.contest #window-length-enabled').text(
          omegaup.UI.formatString(omegaup.T.contestIntroDifferentStarts, {
      ...
      if (contest.scoreboard == 100) {
        $('.contest #scoreboard').text(
          omegaup.T.contestIntroScoreboardTimePercentOneHundred,
        );
      } else if (contest.scoreboard == 0) {
        $('.contest #scoreboard').text(
          omegaup.T.contestIntroScoreboardTimePercentZero,
        );
      } else {
        ...
        $('.contest #scoreboard').text(
          omegaup.UI.formatString(omegaup.T.contestIntroScoreboardTimePercent, {
            window_length: formatTimeInRules(minutesPercentage),
          }),
        );
      }
      $('.contest #submissions-gap').text(
        omegaup.UI.formatString(
      ...
      if (contest.points_decay_factor != 0) {
        $('.contest #points-decay-factor').text(
          omegaup.UI.formatString(omegaup.T.contestNewFormDecrementFactor, {
