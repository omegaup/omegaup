omegaup.OmegaUp.on('ready', function() {
  ...
  $('#start-contest-form').submit(function() {
    ...
    omegaup.API
      .openContest({ contest_alias: contestAlias })
  ...
  function readyToStart(contest) {
    ...
    if (date > contest.finish_time.getTime()) {
      // Ended
      $('#click-to-proceed').removeClass('hidden');
    } else if (date > contest.start_time.getTime()) {
      // Started
      $('#click-to-proceed').removeClass('hidden');
    } else {
      // Not started
  ...
  function contestLoaded(contest) {
    if (contest.status != 'ok') {
      ...
      $('.contest #time-until-start').html(
        omegaup.UI.escape(contest.start_time)
      );
