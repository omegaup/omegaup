omegaup.OmegaUp.on('ready', function() {
  ...
  omegaup.API
    .getCourseAdminDetails({ alias: courseAlias })
    .then(function(course) {
      ...
      $('#start_time', courseForm).val(
        omegaup.UI.formatDate(course.start_time)
      );
      $('#finish_time', courseForm).val(
        omegaup.UI.formatDate(course.finish_time)
      );

      $('#start_time, #finish_time', courseForm).datepicker({
        weekStart: 1,
        format: 'mm/dd/yyyy'
      });
    });
  ...
  courseForm.submit(function() {
    omegaup.API
      .updateCourse({
        ...
        finish_time: (
          new Date($('#finish_time', courseForm).val()).setHours(
            23,
            59,
            59,
            999
      ...
      .then(function(data) {
        omegaup.UI.success(
          'Tu curso ha sido editado! <a href="/course/' +
            ...
            '">' +
            omegaup.T.courseEditGoToCourse +
            '</a>'
        );
        $('.course-header')
...
var koStudentsList = { students: ko.observableArray() };
...
function refreshStudentList() {
  ...
  omegaup.API
    .getCourseStudentList({ course_alias: courseAlias })
    .then(function(data) {
      if (data.status != 'ok') {
      ...
      for (var i = 0; i < data['students'].length; ++i) {
        ...
        var totalHomeworks = data['counts']['homework'] != null
          ...
          : 0;
        var totalTests = data['counts']['test'] != null
          ? data['counts']['test']
          : 0;
        student.totalHomeworks = student.count_homeworks_done +
          '/' +
          totalHomeworks;
