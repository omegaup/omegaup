				...
				if (problem.tags && problem.tags.length > 0) {
					...
					for (var j = 0; j < problem.tags.length; j++) {
						tags.append($('<a class="tag"></a>')
							.attr('href', '/problem/?tag=' + omegaup.UI.escape(problem.tags[j]))
	...
	$("#bulk-make-public").click(function() {
		omegaup.UI.bulkOperation(
			function(alias, handleResponseCallback) {
				omegaup.API.updateProblem(alias, 1 /*public*/, handleResponseCallback);
			},
			function() {
				fillProblemsTable();
			}
	...
	$("#bulk-make-private").click(function() {
		omegaup.UI.bulkOperation(
			function(alias, handleError) {
				omegaup.API.updateProblem(alias, 0 /*public*/, handleResponseCallback);
			},
			function() {
				fillProblemsTable();
			}
