...
def test_create_contest(driver):
    ...
    with driver.login(
        identity.username, identity.password, is_main_user_identity=False
    ):
        create_run_user(
            driver, contest_alias, problem, "Main.cpp17-gcc", verdict="AC", score=1
        )
    ...
    with driver.login_admin():
        driver.wait.until(
            EC.element_to_be_clickable((By.CSS_SELECTOR, "a[data-nav-user]"))
        ).click()

        with driver.page_transition():
            driver.wait.until(
                EC.element_to_be_clickable(
                    (By.CSS_SELECTOR, "a[data-nav-user-contests]")
                )
            ).click()
        ...
        assert_run_verdict(driver, identity.username, problem, classname="accepted")
...
def test_user_ranking_contest(driver):
    ...
    create_contest_admin(
        driver, contest_alias, problem, [user1, user2], driver.user_username
    )
    ...
    with driver.login_admin():
        ...
        util.check_scoreboard_events(
            driver, contest_alias, url, num_elements=3, scoreboard="Public"
        )
        ...
        util.check_scoreboard_events(
            driver, contest_alias, url, num_elements=3, scoreboard="Admin"
        )

        driver.wait.until(
            EC.element_to_be_clickable((By.CSS_SELECTOR, "a[data-nav-contests]"))
        ).click()
        with driver.page_transition():
            driver.wait.until(
                EC.element_to_be_clickable(
                    (By.CSS_SELECTOR, "a[data-nav-contests-arena]")
                )
            ).click()
        ...
        with driver.page_transition():
            ...
            driver.wait.until(
                EC.element_to_be_clickable(
                    (
                        By.CSS_SELECTOR,
                        '.contest-list a[href="/arena/%s/"]' % contest_alias,
                    )
                )
            ).click()

        driver.wait.until(
            EC.element_to_be_clickable((By.XPATH, '//a[@href = "#ranking"]'))
        ).click()
        driver.wait.until(
            EC.visibility_of_element_located((By.CSS_SELECTOR, ".omegaup-scoreboard"))
        )
        ...
        driver.wait.until(
            EC.element_to_be_clickable(
                (By.XPATH, '//input[@class = "toggle-contestants"]')
            )
        ).click()
...
def select_contests_list(driver, selected_list):
    ...
    """
    driver.wait.until(
        EC.element_to_be_clickable((By.CSS_SELECTOR, "a[data-contests]"))
    ).click()
    driver.wait.until(
        EC.element_to_be_clickable((By.CSS_SELECTOR, "a[%s]" % selected_list))
    ).click()
...
def test_user_ranking_contest_when_scoreboard_show_time_finished(driver):
    ...
    with driver.login(user1, password):
        create_run_user(driver, alias, problem, "Main.cpp17-gcc", verdict="AC", score=1)

    with driver.login(user2, password):
        create_run_user(
            driver, alias, problem, "Main_wrong.cpp17-gcc", verdict="WA", score=0
        )
    ...
    with driver.login(driver.user_username, "user"):
        ...
        with driver.page_transition():
            driver.wait.until(
                EC.element_to_be_clickable(
                    (By.XPATH, '//a[starts-with(@href, "%s")]' % contest_url)
                )
            ).click()
        ...
        check_ranking(driver, problem, driver.user_username, scores=["0.00", "+100.00"])
...
def test_user_clarifications_contest(driver):
    ...
    create_contest_admin(driver, contest_alias, problem, [user1], driver.user_username)
...
@util.annotate
def create_contest_admin(
    driver, contest_alias, problem, users, user, access_mode="Public", **kwargs
):
    ...
    with driver.login_admin():
        ...
        assert (
            "/contest/%s/edit/" % contest_alias
        ) in driver.browser.current_url, driver.browser.current_url

        driver.wait.until(
            EC.visibility_of_element_located((By.CSS_SELECTOR, "input[data-title]"))
        ).send_keys("(Updated)")

        driver.browser.find_element(By.CSS_SELECTOR, "form.contest-form").submit()
        ...
        contest_title = driver.browser.find_element(By.CSS_SELECTOR, ".page-header h1")

        assert "(Updated)" in contest_title.text, "Update contest failed"
        assert contest_alias in message_link.get_attribute(
            "href"
        ), "Update contest failed"
        ...
        with driver.page_transition():
            driver.wait.until(
                EC.element_to_be_clickable(
                    (By.XPATH, '//small/a[starts-with(@href, "%s")]' % contest_url)
                )
            ).click()
        assert contest_alias in driver.browser.current_url, driver.browser.current_url

        with driver.page_transition():
            driver.wait.until(
                EC.element_to_be_clickable(
                    (By.CSS_SELECTOR, "button[data-start-contest]")
                )
            ).click()
        driver.wait.until(
            EC.element_to_be_clickable((By.XPATH, '//a[@href = "#ranking"]'))
        ...
        assert (contest_url) in driver.browser.current_url, driver.browser.current_url
...
def create_clarification_user(driver, problem, question):
    ...
    driver.wait.until(
        EC.element_to_be_clickable((By.XPATH, '//a[@href = "#clarifications"]'))
    ).click()
    driver.wait.until(
        EC.visibility_of_element_located((By.CSS_SELECTOR, "[data-tab-clarifications]"))
    )

    driver.wait.until(
        EC.element_to_be_clickable((By.XPATH, '//a[@href = "#clarifications/all/new"]'))
    ).click()

    Select(
        driver.wait.until(
            EC.element_to_be_clickable(
                (By.CSS_SELECTOR, "[data-new-clarification-problem]")
            )
        )
    ).select_by_value(problem)

    driver.wait.until(
        EC.visibility_of_element_located(
            (By.CSS_SELECTOR, "[data-new-clarification-message]")
        )
    ).send_keys(question)

    driver.browser.find_element(By.CSS_SELECTOR, "[data-new-clarification]").submit()

    clarifications = driver.browser.find_elements(
        By.CSS_SELECTOR, "[data-tab-clarifications] table tbody tr"
    )
...
def answer_clarification_admin(driver, answer):
    ...
    Select(
        driver.wait.until(
            EC.element_to_be_clickable((By.CSS_SELECTOR, "[data-select-answer]"))
        )
    ).select_by_value(answer)

    driver.browser.find_element(
        By.CSS_SELECTOR, "[data-form-clarification-answer]"
    ).submit()
...
def create_run_user(driver, contest_alias, problem, filename, **kwargs):
    ...
    driver.wait.until(
        EC.element_to_be_clickable(
            (By.XPATH, ('//a[contains(text(), "%s")]/parent::div' % problem.title()))
        )
    ).click()
    ...
    driver.wait.until(
        EC.element_to_be_clickable((By.CSS_SELECTOR, "button[data-run-details]"))
    ).click()
    assert ("show-run:") in driver.browser.current_url, driver.browser.current_url
...
def create_contest(driver, alias, scoreboard_time_percent=100):
    ...
    with driver.page_transition():
        driver.wait.until(
            EC.element_to_be_clickable((By.CSS_SELECTOR, "a[data-nav-contests-create]"))
        ).click()
    ...
    driver.wait.until(
        EC.visibility_of_element_located((By.CSS_SELECTOR, "button[data-logistics]"))
    ).click()

    driver.browser.find_element(By.NAME, "alias").send_keys(alias)
    driver.browser.find_element(By.NAME, "description").send_keys("contest description")
...
def add_students_contest(driver, users):
    ...
    driver.wait.until(
        EC.element_to_be_clickable((By.CSS_SELECTOR, "a[data-nav-contest-edit]"))
    ).click()
    driver.wait.until(
        EC.visibility_of_element_located((By.XPATH, "//a[@data-nav-contestant]"))
    )
...
def add_students_bulk(driver, users):
    ...
    driver.wait.until(
        EC.element_to_be_clickable((By.CSS_SELECTOR, ("a.contestants")))
    ).click()
    driver.wait.until(
        EC.visibility_of_element_located((By.CSS_SELECTOR, "div.contestants"))
    )

    driver.wait.until(
        EC.visibility_of_element_located(
            (By.XPATH, ('//textarea[contains(@class, "contestants")]'))
        )
    ).send_keys(", ".join(users))
    with util.dismiss_status(driver):
        driver.wait.until(
            EC.element_to_be_clickable((By.CLASS_NAME, ("user-add-bulk")))
        ).click()
    for user in users:
        driver.wait.until(
            EC.visibility_of_element_located(
                (
                    By.XPATH,
                    '//table[contains(@class, "participants")]//a[text()="%s"]' % user,
                )
            )
        )
...
def add_problem_to_contest(driver, problem):
    ...
    driver.wait.until(
        EC.element_to_be_clickable((By.CSS_SELECTOR, "a.problems"))
    ).click()
    ...
    with util.dismiss_status(driver):
        driver.wait.until(
            EC.element_to_be_clickable((By.CSS_SELECTOR, ".btn.add-problem"))
        ).click()
    driver.wait.until(
        EC.visibility_of_element_located(
            (
...
def enter_contest(driver, contest_alias):
    ...
    with driver.page_transition():
        driver.wait.until(
            EC.element_to_be_clickable((By.CSS_SELECTOR, "a[data-nav-contests-arena]"))
        ).click()
    ...
    with driver.page_transition():
        driver.wait.until(
            EC.element_to_be_clickable((By.CSS_SELECTOR, "button[data-start-contest]"))
        ).click()
...
def change_contest_admission_mode(driver, contest_admission_mode):
    ...
    driver.wait.until(
        EC.element_to_be_clickable((By.CSS_SELECTOR, "a.admission-mode"))
    ).click()
    Select(
        driver.wait.until(
            EC.element_to_be_clickable((By.XPATH, '//select[@name = "admission-mode"]'))
        )
    ).select_by_visible_text(contest_admission_mode)
    with util.dismiss_status(driver):
        driver.wait.until(
            EC.element_to_be_clickable((By.CSS_SELECTOR, ".btn.change-admission-mode"))
        ).click()
