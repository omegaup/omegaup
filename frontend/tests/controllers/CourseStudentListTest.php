...
class CourseStudentListTest extends \OmegaUp\Test\ControllerTestCase {
    ...
    public function testGetStudentsProgressForCourse() {
        ...
        print_r($results);
        $allProgress = $results['allProgress'];

        $this->assertEquals($participants[0]->name, $allProgress[0]['name']);
        $this->assertArrayHasKey($assignment, $allProgress[0]['progress']);
        $this->assertEquals(
            100,
            $allProgress[0]['progress'][$assignment][$problemsData[0]['problem']->alias]
        );
        $this->assertEquals(
            100,
            $allProgress[0]['progress'][$assignment][$problemsData[1]['problem']->alias]
        );
        $this->assertEquals(
            0,
            $allProgress[0]['progress'][$assignment][$problemsData[2]['problem']->alias]
        );

        $this->assertEquals($participants[1]->name, $allProgress[1]['name']);
        $this->assertArrayHasKey($assignment, $allProgress[1]['progress']);
        $this->assertEquals(
            0,
            $allProgress[1]['progress'][$assignment][$problemsData[0]['problem']->alias]
        );
        $this->assertEquals(
            100,
            $allProgress[1]['progress'][$assignment][$problemsData[1]['problem']->alias]
        );
        $this->assertEquals(
            0,
            $allProgress[1]['progress'][$assignment][$problemsData[2]['problem']->alias]
        );

        $this->assertEquals($participants[2]->name, $allProgress[2]['name']);
        $this->assertArrayHasKey($assignment, $allProgress[2]['progress']);
        $this->assertEquals(
            0,
            $allProgress[2]['progress'][$assignment][$problemsData[0]['problem']->alias]
        );
        $this->assertEquals(
            0,
            $allProgress[2]['progress'][$assignment][$problemsData[1]['problem']->alias]
        );
        $this->assertEquals(
            100,
            $allProgress[2]['progress'][$assignment][$problemsData[2]['problem']->alias]
        );
    }
