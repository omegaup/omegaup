...
class CourseStudentListTest extends \OmegaUp\Test\ControllerTestCase {
    ...
    /**
     * Regression test for issue #10037: students with a NULL name must not
     * land on the wrong pagination page in apiStudentsProgress.
     *
     * The SQL pagination order must match the PHP usort comparator, which sorts
     * by COALESCE(name, username). With a NULL name, MySQL ORDER BY name would
     * sort the student first (NULLs first), but the PHP comparator treats the
     * null name as the username, so the page boundaries must use the same
     * COALESCE expression or the visible order breaks across pages.
     */
    public function testStudentsProgressPaginationWithNullName() {
        // Course with one assignment and one problem so students have progress rows.
        $courseData = \OmegaUp\Test\Factories\Course::createCourseWithOneAssignment();
        $adminLogin = self::login($courseData['admin']);
        $problemData = \OmegaUp\Test\Factories\Problem::createProblem();
        ...
        [
            ...
            new \OmegaUp\Test\Factories\UserParams(['name' => 'Alice'])
        );
        [
            'identity' => $zoe,
        ] = \OmegaUp\Test\Factories\User::createUser(
        ...
        [
            ...
            new \OmegaUp\Test\Factories\UserParams(['name' => 'Bob'])
        );

        \OmegaUp\Test\Factories\Course::addStudentToCourse($courseData, $alice);
        \OmegaUp\Test\Factories\Course::addStudentToCourse($courseData, $zoe);
        \OmegaUp\Test\Factories\Course::addStudentToCourse($courseData, $bob);

        foreach ([$alice, $zoe, $bob] as $student) {
            $login = self::login($student);
            $run = \OmegaUp\Controllers\Run::apiCreate(new \OmegaUp\Request([
                'auth_token' => $login->auth_token,
        ...
        $page1 = \OmegaUp\Controllers\Course::apiStudentsProgress(
            new \OmegaUp\Request([
                'auth_token' => $login->auth_token,
                'course' => $courseData['course']->alias,
                'page' => 1,
        ...
        $page2 = \OmegaUp\Controllers\Course::apiStudentsProgress(
            new \OmegaUp\Request([
                'auth_token' => $login->auth_token,
                'course' => $courseData['course']->alias,
                'page' => 2,
        ...
        $order = [];
        foreach ([$page1['progress'], $page2['progress']] as $page) {
            foreach ($page as $student) {
                if (is_null($student['name'])) {
                    $order[] = strtolower($student['username']);
                } else {
                    $order[] = strtolower($student['name']);
                }
            }
        }
