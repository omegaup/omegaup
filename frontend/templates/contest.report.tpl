...
{foreach name=outer item=contestantData from=$contestReport}
	...
	{if isset($item.run_details) && isset($item.run_details.details.groups)}
   	<table>
	<tr>
	<th>Case</th>
	<th>Time (Sec)</th>
	...
	<th>Diff</th>
		...
		{foreach item=group from=$item.run_details.details.groups}
			{foreach item=case from=$group.cases}
			<tr>
			<td>{$group.group}.{$case.name}</td>
			<td class="numeric">{$case.meta.time|string_format:"%.3f"}</td>
			<td class="numeric">{$case.meta['wall_time']|string_format:"%.3f"}</td>
			<td class="numeric">{$case.meta.memory|string_format:"%.2f"}</td>
			<td>{$case.verdict}</td>
			<td>{$case.score}</td>
