...
abstract class ContestsDAOBase extends DAO {
    ...
    final public static function search($Contests, $orderBy = null, $orden = 'ASC', $offset = 0, $rowcount = null, $likeColumns = null) {
        ...
        if (!is_null($Contests->last_updated)) {
            $clauses[] = '`last_updated` = ?';
            $params[] = $Contests->last_updated;
        }
        ...
        if (!is_null($Contests->modality)) {
            $clauses[] = '`modality` = ?';
            $params[] = $Contests->modality;
        }
    ...
    final public static function byRange(Contests $ContestsA, Contests $ContestsB, $orderBy = null, $orden = 'ASC') {
        ...
        $a = $ContestsA->last_updated;
        $b = $ContestsB->last_updated;
        if (!is_null($a) && !is_null($b)) {
            $clauses[] = '`last_updated` >= ? AND `last_updated` <= ?';
            $params[] = min($a, $b);
            $params[] = max($a, $b);
        } elseif (!is_null($a) || !is_null($b)) {
            $clauses[] = '`last_updated` = ?';
        ...
        $a = $ContestsA->modality;
        $b = $ContestsB->modality;
        if (!is_null($a) && !is_null($b)) {
            $clauses[] = '`modality` >= ? AND `modality` <= ?';
            $params[] = min($a, $b);
            $params[] = max($a, $b);
        } elseif (!is_null($a) || !is_null($b)) {
            $clauses[] = '`modality` = ?';
