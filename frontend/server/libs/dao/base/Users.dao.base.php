...
abstract class UsersDAOBase extends DAO {
     ...
     * Campos de la tabla.
     */
    const FIELDS = '`Users`.`user_id`, `Users`.`username`, `Users`.`facebook_user_id`, `Users`.`password`, `Users`.`main_email_id`, `Users`.`main_identity_id`, `Users`.`name`, `Users`.`solved`, `Users`.`submissions`, `Users`.`country_id`, `Users`.`state_id`, `Users`.`school_id`, `Users`.`scholar_degree`, `Users`.`language_id`, `Users`.`graduation_date`, `Users`.`birth_date`, `Users`.`gender`, `Users`.`last_access`, `Users`.`verified`, `Users`.`verification_id`, `Users`.`reset_digest`, `Users`.`reset_sent_at`, `Users`.`recruitment_optin`, `Users`.`hide_problem_tags`, `Users`.`in_mailing_list`, `Users`.`is_private`, `Users`.`preferred_language`';
    ...
    final public static function search($Users, $orderBy = null, $orden = 'ASC', $offset = 0, $rowcount = null, $likeColumns = null) {
        ...
        if (!is_null($Users->hide_problem_tags)) {
            $clauses[] = '`hide_problem_tags` = ?';
            $params[] = $Users->hide_problem_tags;
        }
    ...
    final private static function update(Users $Users) {
        $sql = 'UPDATE `Users` SET `username` = ?, `facebook_user_id` = ?, `password` = ?, `main_email_id` = ?, `main_identity_id` = ?, `name` = ?, `solved` = ?, `submissions` = ?, `country_id` = ?, `state_id` = ?, `school_id` = ?, `scholar_degree` = ?, `language_id` = ?, `graduation_date` = ?, `birth_date` = ?, `gender` = ?, `last_access` = ?, `verified` = ?, `verification_id` = ?, `reset_digest` = ?, `reset_sent_at` = ?, `recruitment_optin` = ?, `hide_problem_tags` = ?, `in_mailing_list` = ?, `is_private` = ?, `preferred_language` = ? WHERE `user_id` = ?;';
        $params = [
            $Users->username,
    ...
    final private static function create(Users $Users) {
        ...
        $sql = 'INSERT INTO Users (`user_id`, `username`, `facebook_user_id`, `password`, `main_email_id`, `main_identity_id`, `name`, `solved`, `submissions`, `country_id`, `state_id`, `school_id`, `scholar_degree`, `language_id`, `graduation_date`, `birth_date`, `gender`, `last_access`, `verified`, `verification_id`, `reset_digest`, `reset_sent_at`, `recruitment_optin`, `hide_problem_tags`, `in_mailing_list`, `is_private`, `preferred_language`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);';
        $params = [
            $Users->user_id,
            $Users->username,
            $Users->facebook_user_id,
    ...
    final public static function byRange(Users $UsersA, Users $UsersB, $orderBy = null, $orden = 'ASC') {
        ...
        $a = $UsersA->hide_problem_tags;
        $b = $UsersB->hide_problem_tags;
        if (!is_null($a) && !is_null($b)) {
            $clauses[] = '`hide_problem_tags` >= ? AND `hide_problem_tags` <= ?';
            $params[] = min($a, $b);
            $params[] = max($a, $b);
        } elseif (!is_null($a) || !is_null($b)) {
            $clauses[] = '`hide_problem_tags` = ?';
        ...
        $a = $UsersA->preferred_language;
        $b = $UsersB->preferred_language;
        if (!is_null($a) && !is_null($b)) {
            $clauses[] = '`preferred_language` >= ? AND `preferred_language` <= ?';
            $params[] = min($a, $b);
            $params[] = max($a, $b);
        } elseif (!is_null($a) || !is_null($b)) {
            $clauses[] = '`preferred_language` = ?';
