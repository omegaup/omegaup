...
abstract class CoursesDAOBase extends DAO {
     ...
     * Campos de la tabla.
     */
    const FIELDS = '`Courses`.`course_id`, `Courses`.`name`, `Courses`.`description`, `Courses`.`alias`, `Courses`.`group_id`, `Courses`.`admin_group_id`, `Courses`.`acl_id`, `Courses`.`start_time`, `Courses`.`finish_time`, `Courses`.`public`';
    ...
    final public static function getByPK($course_id) {
        ...
        $sql = 'SELECT `Courses`.`course_id`, `Courses`.`name`, `Courses`.`description`, `Courses`.`alias`, `Courses`.`group_id`, `Courses`.`admin_group_id`, `Courses`.`acl_id`, `Courses`.`start_time`, `Courses`.`finish_time`, `Courses`.`public` FROM Courses WHERE (course_id = ?) LIMIT 1;';
        $params = [$course_id];
        global $conn;
    ...
    final public static function getAll($pagina = null, $columnas_por_pagina = null, $orden = null, $tipo_de_orden = 'ASC') {
        $sql = 'SELECT `Courses`.`course_id`, `Courses`.`name`, `Courses`.`description`, `Courses`.`alias`, `Courses`.`group_id`, `Courses`.`admin_group_id`, `Courses`.`acl_id`, `Courses`.`start_time`, `Courses`.`finish_time`, `Courses`.`public` from Courses';
        global $conn;
        if (!is_null($orden)) {
    ...
    final public static function search($Courses, $orderBy = null, $orden = 'ASC', $offset = 0, $rowcount = null, $likeColumns = null) {
        ...
        if (!is_null($Courses->admin_group_id)) {
            $clauses[] = '`admin_group_id` = ?';
            $params[] = $Courses->admin_group_id;
        }
    ...
    final private static function update(Courses $Courses) {
        $sql = 'UPDATE `Courses` SET `name` = ?, `description` = ?, `alias` = ?, `group_id` = ?, `admin_group_id` = ?, `acl_id` = ?, `start_time` = ?, `finish_time` = ?, `public` = ? WHERE `course_id` = ?;';
        $params = [
            $Courses->name,
            $Courses->description,
            $Courses->alias,
            $Courses->group_id,
            $Courses->admin_group_id,
            $Courses->acl_id,
            $Courses->start_time,
            $Courses->finish_time,
            $Courses->public,
    ...
    final private static function create(Courses $Courses) {
        ...
        $sql = 'INSERT INTO Courses (`course_id`, `name`, `description`, `alias`, `group_id`, `admin_group_id`, `acl_id`, `start_time`, `finish_time`, `public`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);';
        $params = [
            $Courses->course_id,
            $Courses->name,
            $Courses->description,
    ...
    final public static function byRange(Courses $CoursesA, Courses $CoursesB, $orderBy = null, $orden = 'ASC') {
        ...
        $a = $CoursesA->admin_group_id;
        $b = $CoursesB->admin_group_id;
        if (!is_null($a) && !is_null($b)) {
            $clauses[] = '`admin_group_id` >= ? AND `admin_group_id` <= ?';
            $params[] = min($a, $b);
            $params[] = max($a, $b);
        } elseif (!is_null($a) || !is_null($b)) {
            $clauses[] = '`admin_group_id` = ?';
