...
class RunsDAO extends RunsDAOBase {
				...
				INNER JOIN
					(
						...
						FROM
							Runs rr
						WHERE
							rr.problem_id = ? AND rr.verdict = \'AC\' AND rr.test = \'normal\' GROUP BY rr.identity_id
					) AS sr ON sr.identity_id = r.identity_id AND sr.runtime = r.runtime
				WHERE
					r.problem_id = ? AND r.verdict = \'AC\' AND r.test = \'normal\'
				GROUP BY
    ...
    final public static function GetAllRuns($problemset_id, $status, $verdict, $problem_id, $language, $identity_id, $offset, $rowcount) {
        $sql = 'SELECT r.run_id, r.guid, r.language, r.status, r.verdict, r.runtime, r.penalty, ' .
                ...
                'r.submit_delay, r.test, i.username, p.alias, i.country_id, c.alias AS contest_alias ' .
    ...
    final public static function CountTotalRunsOfProblemInProblemset($problem_id, $problemset_id) {
        // Build SQL statement.
        $sql = 'SELECT COUNT(*) FROM Runs WHERE problem_id = ? AND problemset_id = ? AND test = \'normal\'';
            ...
            $sql = '
                ...
                INNER JOIN
                    ...
                    WHERE
                        ...
                        r.problemset_id = ? AND
                        r.status = \'ready\' AND
                        r.test = \'normal\') rc ON i.identity_id = rc.identity_id';
            $val = [$problemset_id];
    ...
    final public static function GetBestRun($problemset_id, $problem_id, $identity_id, $showAllRuns) {
        $filterTest = $showAllRuns ? '' : ' AND test = \'normal\'';
        $sql = "
            SELECT
