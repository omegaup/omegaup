...
class ContestController extends Controller {
	...
	public static function apiUpdate(Request $r) {
		...
		$valueProperties = array(
			"title",
			"description",
			"start_time"		=> ["StartTime", false, function($value) { return gmdate('Y-m-d H:i:s', $value); }],
			"finish_time"		=> ["FinishTime", false, function($value) { return gmdate('Y-m-d H:i:s', $value); }],
			"window_length" => ["WindowLength", false, function($value) { return $value == "NULL" ? NULL : $value; }],
			"scoreboard",
			"points_decay_factor",
			"partial_score",
			"submissions_gap",
			"feedback",
			"penalty"				=> ["Penalty", false, function($value) { return max(0, intval($value)); }],
			"penalty_time_start",
			"penalty_calc_policy",
