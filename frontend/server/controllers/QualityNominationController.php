...
class QualityNominationController extends Controller {
    ...
    public static function apiAggreateFeedback(Request $r) {
        ...
        foreach($problemTable as $key => $row) {
            ...
            $problem->quality = bayesianAverage($globalQualityAverage, $row['quality_sum'], $row['quality_n']);
            $problem->difficulty = bayesianAverage($globalDifficultyAverage, $row['difficulty_sum'], $row['difficulty_n']);
            // TODO(heduenas): Get threshold parameter from DB for each problem independently.
            $tags = mostVotedTags($row['tags'], 0.25);
            ProblemsDAO::save($problem);
            ProblemsTags::replaceAutogeneratedTags($problem, $tags);
        }
    }
    ...
    private static function bayesianAverage($aprioriAverage, $sum, $n) {
        $C = 5;
        if ($n >= $C) {
            return ($C * $aprioriAverage + $sum) / ($C + $n);
        }
        return null;
    }
