...
class ProblemController extends Controller {
    ...
    private static function validateCreateOrUpdate(Request $r, $is_update = false) {
        ...
        if ($is_update) {
            ...
            $r['selected_tags'] = (array)json_decode($r['selected_tags']);
            foreach ($r['selected_tags'] as $tag) {
                if (count((array)$tag) != 2 || !property_exists($tag, 'tagname') || !property_exists($tag, 'public')) {
                    throw new InvalidParameterException('invalidParameters', 'tags');
                }
            }
        }
    ...
    private static function addTag($tagName, $isPublic, $problemId) {
        // Normalize name.
        Validators::isStringNonEmpty($tagName, 'name');
        ...
        $problemTag->problem_id = $problemId;
        $problemTag->tag_id = $tag->tag_id;
        $problemTag->public = $isPublic;
        $problemTag->autogenerated = 0;
