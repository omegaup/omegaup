...
class Problems extends \OmegaUp\DAO\Base\Problems {
    ...
    /**
     * @return array{name: string, autogenerated: bool}[]
     */
    final public static function getTagsForProblem(\OmegaUp\DAO\VO\Problems $problem, bool $public) {
        $sql = 'SELECT
            t.name,
            pt.autogenerated
        FROM
        ...
        /** @var array{name: string, autogenerated: bool}[] $rs */
        $rs = \OmegaUp\MySQLConnection::getInstance()->GetAll($sql, [$problem->problem_id]);
        $result = [];

        foreach ($rs as $r) {
            $result[] = ['name' => $r['name'], 'autogenerated' => boolval($r['autogenerated'])];
        }
