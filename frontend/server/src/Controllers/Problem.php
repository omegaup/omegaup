    ...
    public static function getCollectionsDetailsForSmarty(\OmegaUp\Request $r): array {
        ...
        if ($collectionType === 'author') {
            ...
            foreach ($response['results'] as $problem) {
                if (
                    $problem['tags'][0]['name'] === $r->ensureString(
                        'collection_type'
                    )
                ) {
                    $problems[] = $problem;
