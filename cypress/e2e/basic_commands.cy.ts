...
describe('Basic Commands Test', () => {
  ...
  it.only('Should create two contest and merge their scoreboard', () => {
    ...
    cy.login(loginOptions[0]);
    contestPage.mergeContests([contestOptions1.contestAlias, contestOptions2.contestAlias]);
    contestPage.verifyMergedScoreboard([loginOptions[0].username, loginOptions[3].username]);
    cy.logout();
  });
});
