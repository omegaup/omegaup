...
class DatabaseUtilsTest(unittest.TestCase):
    ...
    def test_inside_container_true(self) -> None:
        '''inside_container returns True when /opt/omegaup directory exists.'''
        with mock.patch('os.path.isdir', return_value=True):
            self.assertTrue(database_utils.inside_container())

    def test_inside_container_false(self) -> None:
        '''inside_container returns False when /opt/omegaup directory does not exist.'''
        with mock.patch('os.path.isdir', return_value=False):
            self.assertFalse(database_utils.inside_container())

    def test_check_inside_container_when_inside(self) -> None:
        '''check_inside_container returns immediately if inside container.'''
        with mock.patch.object(database_utils, 'inside_container', return_value=True):
    ...
    def test_check_inside_container_user_declines(self) -> None:
        ...
        with mock.patch.object(database_utils, 'inside_container', return_value=False):
    ...
    def test_check_inside_container_user_accepts(self) -> None:
        '''check_inside_container runs docker compose as separate arguments when user answers y.'''
        fake_completed = mock.Mock(spec=subprocess.CompletedProcess, returncode=0)
        with mock.patch.object(database_utils, 'inside_container', return_value=False):
            with mock.patch('sys.stdin.isatty', return_value=True):
                with mock.patch('builtins.input', return_value='y'):
                    with mock.patch('subprocess.run', return_value=fake_completed) as mock_run:
                        with mock.patch.object(sys, 'argv', ['db-migrate.py', 'validate']):
                            with self.assertRaises(SystemExit) as exc_info:
                                database_utils.check_inside_container()
                            self.assertEqual(exc_info.exception.code, 0)
                            mock_run.assert_called_once_with(
                                ['docker', 'compose', 'exec', '-T', 'frontend', 'db-migrate.py', 'validate'],
                                check=False,
                            )

    def test_mysql_outside_container(self) -> None:
        '''mysql() prepends separate docker compose arguments when outside container.'''
        with mock.patch.object(database_utils, 'inside_container', return_value=False):
            with mock.patch('subprocess.check_output', return_value='1\n') as mock_output:
                ...
                mock_output.assert_called_once_with(
                    [
                        ...
                        '/usr/bin/mysql', '--user=test', 'omegaup', '-NBe', 'SELECT 1;',
                    ],
    ...
    def test_mysql_inside_container(self) -> None:
        '''mysql() does not prepend docker compose when inside container.'''
        with mock.patch.object(database_utils, 'inside_container', return_value=True):
            with mock.patch('subprocess.check_output', return_value='1\n') as mock_output:
                result = database_utils.mysql('SELECT 1;', container_check=True)
                self.assertEqual(result, '1\n')
                mock_output.assert_called_once_with(
    ...
    def test_mysqldump_outside_container(self) -> None:
        '''mysqldump() prepends separate docker compose arguments when outside container.'''
        with mock.patch.object(database_utils, 'inside_container', return_value=False):
            with mock.patch('subprocess.check_call') as mock_call:
                with mock.patch('tempfile.NamedTemporaryFile') as mock_temp:
                    ...
                    self.assertEqual(called_args[:5], ['docker', 'compose', 'exec', '-T', 'frontend'])
    ...
    def test_mysqldump_inside_container(self) -> None:
        '''mysqldump() does not prepend docker compose when inside container.'''
        with mock.patch.object(database_utils, 'inside_container', return_value=True):
            with mock.patch('subprocess.check_call') as mock_call:
                with mock.patch('tempfile.NamedTemporaryFile') as mock_temp:
                    mock_file = mock.MagicMock()
    ...
    def test_quote(self) -> None:
        ...
        self.assertIn(database_utils.quote('with space'), ("'with space'", '"with space"'))

    def test_authentication_with_config_file(self) -> None:
        '''authentication() returns --defaults-file when file exists.'''
        with mock.patch('os.path.isfile', return_value=True):
            auth_flags = database_utils.authentication(config_file='/tmp/my.cnf')
            self.assertTrue(auth_flags == ["--defaults-file='/tmp/my.cnf'"] or auth_flags == ['--defaults-file=/tmp/my.cnf'])

    def test_authentication_with_credentials(self) -> None:
        '''authentication() returns username, password, host, and port flags.'''
        with mock.patch('os.path.isfile', return_value=False):
            ...
            self.assertTrue("--user='dbuser'" in auth_flags or '--user=dbuser' in auth_flags)
            self.assertTrue("--password='secretpassword'" in auth_flags or '--password=secretpassword' in auth_flags)
            self.assertIn('--protocol=TCP', auth_flags)
            self.assertTrue("--host='dbhost'" in auth_flags or '--host=dbhost' in auth_flags)
