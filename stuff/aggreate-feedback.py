...
def getProblemAggregates(db, problemId):
    cur = db.cursor()
    print ("QualityNominations`.`contents` FROM `QualityNominations` WHERE `nomination` = 'suggestion' AND `QualityNominations`.`problem_id` = %s;" % problemId)
...
def mostVotedTags(problemTagVotes, problemTagVotesN):
    if problemTagVotesN < 5:
        return None
    maximum = max(stats, key=problemTagVotes.get)
...
def replaceAutogeneratedTags(db, problemId, problemTags):
    ...
    for tag in problemTags:
        ...
        db.query("INSERT INTO Problems_Tags (`problem_id`, `tag_id`, `public`, `autogenerated`) VALUES (%s, %s, %s, %s);",
                problemId, tagId, 1, 1)
        db.commit()
