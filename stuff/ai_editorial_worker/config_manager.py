...
Handles loading configuration from files (production) and environment variables
(dev/testing). Follows omegaUp's pattern of using config files for secure
...
class AIConfigManager:
    ...
    Loads configuration from files (production) and environment variables
    (dev/testing). Follows omegaUp's pattern for secure credential management.
    """

    def __init__(
        self,
        config_file: Optional[str] = None,
        ai_config_file: Optional[str] = None):
        ...
        self.ai_config_file = (
            ai_config_file or self._default_ai_settings_file_path())
    ...
    def _load_ai_settings(self) -> None:
        ...
        if os.path.exists(self.ai_config_file):
            try:
                ...
                logging.info("Loaded AI settings from %s", self.ai_config_file)
            except Exception as e:  # pylint: disable=broad-except
                # Catch all exceptions for graceful config fallback
                logging.warning(
                    "Failed to load AI settings from %s: %s",
                    self.ai_config_file, e)
    ...
    def _load_credentials(self) -> None:
        """Load credentials from config file (production) or environment
        (dev)."""
        self.credentials = {}

        # Try to load from config file first (production pattern)
        if self.config_file and os.path.exists(self.config_file):
            try:
                config = configparser.ConfigParser()
                config.read(self.config_file)

                # Follow cronjob pattern: load from sections, strip quotes
                # like database credentials
                # Check for AI-specific sections first, then fallback to
                # general sections
                if 'ai_openai' in config:
                    self.credentials['openai'] = {
                        'api_key': config['ai_openai'].get(
                            'api_key',
                            '').strip("'\""),
                        'model': config['ai_openai'].get(
                            'model',
                            'gpt-4o').strip("'\""),
                        'rpm_limit': int(
                            config['ai_openai'].get(
                                'rpm_limit',
                                '500')),
                        'tpm_limit': int(
                            config['ai_openai'].get(
                    ...
                    self.credentials['openai'] = {
                        'api_key': config['openai'].get(
                            'api_key',
                            '').strip("'\""),
                        'model': config['openai'].get(
                            'model',
                            'gpt-4o').strip("'\""),
                        'rpm_limit': int(
                            config['openai'].get(
                ...
                if 'ai_deepseek' in config:
                    self.credentials['deepseek'] = {
                        'api_key': config['ai_deepseek'].get(
                            'api_key',
                            '').strip("'\""),
                        'model': config['ai_deepseek'].get(
                            'model',
                            'deepseek-chat').strip("'\""),
                        'rpm_limit': int(
                            config['ai_deepseek'].get(
                                'rpm_limit',
                                '300')),
                        'tpm_limit': int(
                            config['ai_deepseek'].get(
                    ...
                    self.credentials['deepseek'] = {
                        'api_key': config['deepseek'].get(
                            'api_key',
                            '').strip("'\""),
                        'model': config['deepseek'].get(
                            'model',
                            'deepseek-chat').strip("'\""),
                        'rpm_limit': int(
                            config['deepseek'].get(
                ...
                if 'ai_anthropic' in config:
                    self.credentials['anthropic'] = {
                        'api_key': config['ai_anthropic'].get(
                            'api_key',
                            '').strip("'\""),
                        'model': config['ai_anthropic'].get(
                            'model',
                            'claude-3-sonnet-20240229').strip("'\""),
                        'rpm_limit': int(
                            config['ai_anthropic'].get(
                                'rpm_limit',
                                '100')),
                        'tpm_limit': int(
                            config['ai_anthropic'].get(
                ...
                if 'ai_google' in config:
                    self.credentials['google'] = {
                        'api_key': config['ai_google'].get(
                            'api_key',
                            '').strip("'\""),
                        'model': config['ai_google'].get(
                            'model',
                            'gemini-pro').strip("'\""),
                        'rpm_limit': int(
                            config['ai_google'].get(
                                'rpm_limit',
                                '60')),
                        'tpm_limit': int(
                            config['ai_google'].get(
                    ...
                    self.credentials['google'] = {
                        'api_key': config['google'].get(
                            'api_key',
                            '').strip("'\""),
                        'model': config['google'].get(
                            'model',
                            'gemini-pro').strip("'\""),
                        'rpm_limit': int(
                            config['google'].get(
                ...
                logging.info(
                    "Loaded credentials from config file: %s",
                    self.config_file)
    ...
    def get_llm_config(self, provider: Optional[str] = None) -> Dict[str, Any]:
        ...
        if not provider:
            ...
            if ('openai' in self.credentials and
                self.credentials['openai'].get('api_key')):
                provider = 'openai'  # Map to 'gpt' in editorial_generator
            elif ('deepseek' in self.credentials and
                  self.credentials['deepseek'].get('api_key')):
                provider = 'deepseek'
            elif ('anthropic' in self.credentials and
                  self.credentials['anthropic'].get('api_key')):
                provider = 'anthropic'  # Map to 'claude'
            elif ('google' in self.credentials and
                  self.credentials['google'].get('api_key')):
                provider = 'google'  # Map to 'gemini' in editorial_generator
            else:
        ...
        return {
            'provider': provider,  # Keep original name for mapping
    ...
    def load_all_prompts(self) -> Dict[str, str]:
        ...
        for prompt_name, filename in prompt_files.items():
            ...
            try:
                ...
                logging.debug("Loaded prompt template: %s", prompt_name)
            except FileNotFoundError:
                logging.warning("Prompt template not found: %s", file_path)
                prompts[prompt_name] = self._get_default_prompt(prompt_name)
            except Exception as e:  # pylint: disable=broad-except
                # Catch all exceptions for graceful prompt fallback
                logging.error(
                    "Error loading prompt template %s: %s", file_path, e)
                prompts[prompt_name] = self._get_default_prompt(prompt_name)
    ...
    def _get_default_prompt(self, prompt_name: str) -> str:
        ...
        defaults = {
            'editorial_generation': (
                "Write a clear editorial for this competitive programming "
                "problem."),
            'solution_generation': (
                "Generate a solution for this competitive programming "
                "problem."),
    ...
    def configure_parser(parser: argparse.ArgumentParser) -> None:
        """Add AI config arguments to argument parser (similar to lib.db
        pattern)."""
...
def create_example_config_file() -> None:
...
"""
    ...
    print("Example config file created at: %s" % example_file)
