...
for lang in languages:
	...
	with open(js_lang_path, 'w') as lang_file:
		...
		lang_file.write("var omegaup = require('../dist/omegaup.js');\n\n")
		lang_file.write('omegaup.OmegaUp.loadTranslations({\n')
		for key in sorted(strings.keys()):
		    lang_file.write('\t%s: %s,\n' % (key, json.dumps(strings[key][lang])))
		lang_file.write('});\n')
	json_lang_path = os.path.join(js_templates_dir, 'lang.%s.json' % lang)
	with open(json_lang_path, 'w') as lang_file:
		json.dump(json_map, lang_file, sort_keys=True, indent='\t')
