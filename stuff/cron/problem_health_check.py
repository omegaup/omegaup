...
def find_judge_errors(
    ...
    '''Finds the problems behind the judge errors in the recent runs.'''
    cur.execute(
        '''
        SELECT
            s.`problem_id`,
    ...
    return [
        Finding(
            ...
            detail=(f'{int(row["error_count"])} of the last {sample} runs '
                    f'on the site failed to be judged'),
...
def apply_findings(
    ...
    The clock is read before any write, so a finding stored now is never
    mistaken for one that went missing.
    '''
    run_timestamp = current_timestamp(dbconn)
    record_findings(dbconn, findings, run_timestamp)
    return resolve_missing_findings(dbconn, run_timestamp)
    ...
    with lib.runner.run(parser.prog, args) as cron_run:
        ...
        try:
            ...
            with cron_run.phase('record_findings'):
                resolved = apply_findings(dbconn, findings)
